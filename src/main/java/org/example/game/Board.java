...
public class Board {
    ...
    public void placeMove(Player player, int move) {
        ...
        board[row][column] = player.getMoveSymbol().getSymbol();
        remainingMoves--;
    }
    ...
    private boolean checkRows(final Player player) {
        ...
        for(int row = 0 ; row < boardSize; ++row){
            ...
            for(int column = 0 ; column < boardSize; ++column){
                ...
                if (board[row][column] == player.getMoveSymbol().getSymbol()) {
    ...
    private boolean checkColumns(final Player player) {
        ...
        for(int row = 0 ; row < boardSize; ++row){
            ...
            for(int column = 0 ; column < boardSize; ++column){
                ...
                if (board[column][row] == player.getMoveSymbol().getSymbol()) {
    ...
    private boolean checkDiagonals(final Player player) {
        ...
        for(int row = 0 ; row < boardSize; ++row){
            ...
            if (board[row][row] == player.getMoveSymbol().getSymbol()) {
